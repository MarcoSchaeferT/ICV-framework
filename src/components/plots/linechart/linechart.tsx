...
import { getGridCellIndex, getGridCellDims, polygonParser, getGeometryCenter, getGoodReadableRange } from '../maps/helpers';
import {
  metaDataT,
...
const LinechartComponent = ({chartProps}: {chartProps: LinechartProps}) => {
  ...
  *********************/
  ...
  useEffect(() => {
    ...
    if(c.curMonth == -1) {
     const curMonthNumber =  c.curFeature.split("_").length > 1 && !c.curFeature.includes("prob") ? parseInt(c.curFeature.split("_")[1]) : -1;
     if(curMonthNumber !== -1) {
...
const [NminY, NmaxY, ticks] = useMemo(() => {
    ...
    if (props.yDomain) {
      ...
      // Use getGoodReadableRange to match the ColorMapLegend scaling
      const rangeValues = getGoodReadableRange(dataMin, dataMax);
      minY = rangeValues[0];
      maxY = rangeValues[1];
    }
    
    if (minY === maxY) {
        maxY = minY + 1; // Prevent infinite loop if data has no variance
    }
...
const ChartHeader = ({ ErrorData, props }: { ErrorData: string[], props: LinechartProps }) => {
...
const SelectFilterElement = ({ interfacContext, props }: { interfacContext: interfaceContextI, props: LinechartProps }) => {
   let c = interfacContext;
